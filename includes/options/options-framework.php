...
function themezee_options_add_theme_page() {
	add_theme_page(__('Theme Options', 'zeeFocus_language'), __('Theme Options', 'zeeFocus_language'), 'edit_theme_options', 'themezee', 'themezee_options_page');
}
...
function themezee_options_scripts() { 
	...
	if ( isset($_GET['page']) and $_GET['page'] == 'themezee' ) :
		...
		wp_localize_script('themezee_options_js', 'zee_localizing_upload_js', array(
			'use_this_image' => __('Use this Image', 'zeeFocus_language')
		));
...
?>
	...
	<div class="wrap zee_admin_wrap">  			
		...
		<div id="zee_admin_heading">
			...
			<h2><?php echo $theme_data->Name; ?> <?php _e('Theme Options', 'zeeFocus_language'); ?></h2>
		</div>
		<div class="clear"></div>
		<?php if ( isset( $_GET['settings-updated'] ) ) : ?>
			<div class="updated"><p><?php _e('Theme Options successfully updated.', 'zeeFocus_language'); ?></p></div>
		<?php endif; ?>
		<div class="clear"></div>
		...
		?>
			...
			<form class="zee_form" action="options.php" method="post">
					...
					<div class="zee_settings">
						<?php settings_fields('zeefocus_options'); ?>
						<?php do_settings_sections('themezee'); ?>
					</div>

				<input name="zeefocus_options[validation-submit]" type="hidden" value="<?php echo $tab ?>" />

				<p><input name="Submit" class="button-primary" type="submit" value="<?php esc_attr_e('Save Changes', 'zeeFocus_language'); ?>" /></p>
			</form>
...
function themezee_options_tabs_menu() {
	...
	$links = array();
	if ( $current == 'welcome' ) :
		$links[] = '<a class="nav-tab nav-tab-active" href="?page=themezee&tab=welcome">'. __('Welcome', 'zeeFocus_language') .'</a>';
	else :
		$links[] = '<a class="nav-tab" href="?page=themezee&tab=welcome">'. __('Welcome', 'zeeFocus_language') .'</a>';
	endif;
...
function themezee_options_register_settings() {
	...
	register_setting( 'zeefocus_options', 'zeefocus_options', 'themezee_options_validate' );
...
function themezee_options_display_setting( $setting = array() ) {
	...
	switch ( $setting['type'] ) {
		...
		case 'text':
			echo "<input id='".$setting['id']."' class='zee-text-field' name='zeefocus_options[".$setting['id']."]' type='text' value='". esc_attr($options[$setting['id']]) ."' />";
			echo '<br/><label>'.$setting['desc'].'</label>';
		break;
		
		case 'url':
			echo "<input id='".$setting['id']."' class='zee-url-field' name='zeefocus_options[".$setting['id']."]' type='text' value='". esc_url($options[$setting['id']]) ."' />";
			echo '<br/><label>'.$setting['desc'].'</label>';
		break;
		
		case 'textarea':
			echo "<textarea id='".$setting['id']."' name='zeefocus_options[".$setting['id']."]' rows='5'>" . esc_attr($options[$setting['id']]) . "</textarea>";
			echo '<br/><label>'.$setting['desc'].'</label>';
		break;
		
		case 'html':
			echo "<textarea id='".$setting['id']."' name='zeefocus_options[".$setting['id']."]' rows='5'>" . wp_kses_post($options[$setting['id']]) . "</textarea>";
			echo '<br/><label>'.$setting['desc'].'</label>';
		break;
		...
		case 'editor':
			$editor_settings = array(
				'media_buttons' => false,
				'teeny' => true,
				'textarea_name' => 'zeefocus_options['.$setting['id'].']',
				'textarea_rows' => 10,
				'quicktags' => true
			);
		...
		case 'checkbox':
			echo "<input id='".$setting['id']."' name='zeefocus_options[".$setting['id']."]' type='checkbox' value='true'";
			checked( $options[$setting['id']], 'true' );
			echo ' /><label> '.$setting['desc'].'</label>';
		break;
		
		case 'multicheckbox':
			echo "<input id='".$setting['id']."' name='zeefocus_options[".$setting['id']."]' type='hidden' value='true' />";
			foreach ( $setting['choices'] as $value => $label ) {
				...
				echo " type='checkbox' name='zeefocus_options[".$checkbox."]' value='true'/> " . $label . "<br/>";
			}
			echo '<label>'.$setting['desc'].'</label>';
		break;
	
		case 'select':
			echo "<select id='".$setting['id']."' name='zeefocus_options[".$setting['id']."]'>";
		...
		case 'radio':
			foreach ( $setting['choices'] as $value => $label ) {
				...
				echo " type='radio' name='zeefocus_options[".$setting['id']."]' value='" . $value . "'/> " . $label . "<br/>";
			}
			echo '<label>'.$setting['desc'].'</label>';
		break;
		...
		case 'image':
			...
			echo '<input class="zee-upload-image-field" id="'.$setting['id'].'" name="zeefocus_options['.$setting['id'].']" type="text" value="'. esc_attr($options[$setting['id']]) .'" />';
			echo '<input class="zee-upload-image-button button-secondary" type="button" value="'. __('Upload Image', 'zeeFocus_language') .'" />';
			echo '	<label>'.$setting['desc'].'</label>';
		break;

		case 'fontpicker':
			echo "<select id='".$setting['id']."' class='zee-fontpicker' name='zeefocus_options[".$setting['id']."]'>";
				foreach ( $setting['choices'] as $value => $label ) {
		...
		case 'colorpicker':
			echo "<input id='".$setting['id']."' name='zeefocus_options[".$setting['id']."]' class='zee-colorpicker-field' type='text' value='". esc_attr($options[$setting['id']]) ."' data-default-color='".$setting['std']."' />";
			echo '<br/><label>'.$setting['desc'].'</label>';
		break;
		
		default:
			echo "<input id='".$setting['id']."' class='zee-text-field' name='zeefocus_options[".$setting['id']."]' size='40' type='text' value='". esc_attr($options[$setting['id']]) ."' />";
			echo '<br/><label>'.$setting['desc'].'</label>';
		break;
	}
}
...
function themezee_options_set_default_options() {
     $theme_options = get_option( 'zeefocus_options' );
     ...
     update_option( 'zeefocus_options', $theme_options );
}
